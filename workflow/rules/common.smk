...
try:
    ...
    log_file_path = Path("logs/cellranger_multi/cellranger_conda_bin.txt")
    ...
    try:
        ...
        CELLRANGER_VERSION = Version(
            shell(f"{cellranger_bin} --version", read=True)
            .strip()
            .replace("cellranger ", "")
            .replace("cellranger-", "")
        )
    ...
    for pool in ALL_IDS:
        ...
        for ft in feature_types:
            ...
            if ft == "Gene Expression":
                ...
                # request per-pool summaries, collections and per-sample
                # outputs, which only CELLRANGER >= 10.0.0 produces
