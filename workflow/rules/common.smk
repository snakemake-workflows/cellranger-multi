...
TARBALL_VERSION = Version(CELLRANGER_TARBALL.split("/cellranger-")[-1].replace(".tar.gz",""))

try:
    # This file gets generated by cellranger.post-deploy.sh upon installation
    # of cellranger from a tarball.
    log_file_path = Path("logs/cellranger/cellranger_conda_bin.txt")    
    previous_cellranger_install = log_file_path.resolve(strict=True)
except FileNotFoundError:
    # If the file does not yet exist, we have to parse the version number from
    # the CELLRANGER_TARBALL environment variable, as this one will get
    # installed now.
    print(
    ...
    # environment.
    print(f"Found an existing cellranger installation, will use that version unless the environment definition or the post-deploy script changed.")
    with open(previous_cellranger_install) as file:
        for line in file:
            if not line.startswith("#"):
                cellranger_bin=line.strip()
    try:
        Path(cellranger_bin).resolve(strict=True)
    except FileNotFoundError:
        # Again, we fall back to parsing the version number from the
        # CELLRANGER_TARBALL environment variable, as this one will get
        # installed now.
        print(
        ...
        # the version string for 9.0.1 is `cellranger cellranger-9.0.1`, the one for
        # 10.0.0 is `cellranger 10.0.0` --- AAAAARRRRRRGH
        CELLRANGER_VERSION=Version(shell(f"{cellranger_bin} --version", read=True).strip().replace("cellranger ", "").replace("cellranger-", ""))
